...
- **`src/Payment/Payment_Data_Resolver.php`** — produces the merge-tag context arrays. It never references payments' classes directly; it reads order/subscription context through the public `leastudios_payments_*_email_context` and `leastudios_payments_local_subscription_id` filters that the payments plugin answers (`src/Support/Email_Context_Provider.php` on the payments side). When payments is inactive nobody answers the filters, the resolver gets its `null` default, and the resolve methods return their empty shapes. The two array shapes (`OrderEmailContext`, `SubscriptionEmailContext`) are declared as local `@phpstan-type` annotations and asserted by `tests/ContractTest.php` — this is the static-typing seam that replaces the old cross-repo PHPStan scan.
...
This plugin has **no build-time dependency** on `leastudios-payments`. It can lint, test, and package with the payments repo absent.

- The integration is purely runtime, over hooks: `Payment_Email_Listener` reacts to payments' `do_action` events, and `Payment_Data_Resolver` reads context through the public `leastudios_payments_order_email_context`, `leastudios_payments_subscription_email_context`, and `leastudios_payments_local_subscription_id` filters that payments answers (producer: `LEAStudios\Payments\Support\Email_Context_Provider`). Each filter returns a plain scalar array — never a payments object.
- `phpstan.neon` does **not** scan `../leastudios-payments/src`, and the CI `lint` job does **not** check out the payments repo. Do not re-add either: the typed contract now lives in the local `@phpstan-type` annotations on `Payment_Data_Resolver` plus `tests/ContractTest.php`, which asserts the runtime array shapes match those annotations.
- The runtime guard is `Plugin::is_payments_active()` (which checks `LEASTUDIOS_PAYMENTS_VERSION`). When payments is inactive the filters go unanswered, the resolver receives its `null` default, and every resolve method returns its empty/default shape — so the feature degrades gracefully.
