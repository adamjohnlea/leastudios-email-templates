 ...
 *
 * Variables in scope:
 *   string $leastudios_email_templates_email       Suppressed email (normalized).
 *   string $leastudios_email_templates_token       Signed token (for the resubscribe form).
 *   string $leastudios_email_templates_button_bg   Resolved brand color for the resubscribe button (6-digit hex).
 *   string $leastudios_email_templates_button_text Resolved text color for the button (#111827 or #ffffff).
 *
...
<head>
	...
	<style>
		...
		button { font: inherit; cursor: pointer; background: <?php echo esc_attr( $leastudios_email_templates_button_bg ); ?>; color: <?php echo esc_attr( $leastudios_email_templates_button_text ); ?>; border: 0; border-radius: 6px; padding: 10px 16px; font-weight: 600; transition: opacity 0.15s ease; }
...
<body>
	<div class="card">
		...
		<form method="POST" action="<?php echo esc_url( rest_url( 'leastudios-email-templates/v1/resubscribe' ) ); ?>">
			<input type="hidden" name="token" value="<?php echo esc_attr( $leastudios_email_templates_token ); ?>">
